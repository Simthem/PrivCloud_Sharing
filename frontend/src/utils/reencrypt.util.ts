    ...
    fileLoop: for (const file of share.files) {
      ...
      for (let fileAttempt = 0; fileAttempt <= MAX_FILE_RETRIES; fileAttempt++) {
        try {
          ...
          if (e?.message?.includes("annulé")) throw e;
...
export async function reencryptTeam(
  ...
  try {
    ...
    try {
      const configs = (
        await fetch("/api/configs", { credentials: "include" }).then((r) => r.json())
      ) as { key: string; value?: string; defaultValue?: string }[];
      const cfg = configs.find((c) => c.key === "share.chunkSize");
      if (cfg) configChunkSize = parseInt(cfg.value ?? cfg.defaultValue ?? "10000000");
    } catch { /* fallback */ }
    ...
    for (const share of teamShares) {
      ...
      fileLoop: for (const file of share.files) {
        if (signal?.aborted) throw new Error("Re-chiffrement annulé par l'utilisateur");
        ...
        for (let fileAttempt = 0; fileAttempt <= MAX_FILE_RETRIES; fileAttempt++) {
          try {
            const response = await fetchStreaming(`/api/shares/${share.id}/files/${file.id}`);
            ...
            const parts: Uint8Array[] = [];
            let totalDecrypted = 0;
            for await (const chunk of decryptStream(response.body, oldKey, configChunkSize, totalSize)) {
              parts.push(chunk);
              totalDecrypted += chunk.length;
            }
            ...
            const totalChunks = Math.max(1, Math.ceil(totalDecrypted / REENCRYPT_CHUNK_SIZE));

            for (let i = 0; i < totalChunks; i++) {
              if (signal?.aborted) throw new Error("Re-chiffrement annulé par l'utilisateur");
              const start = i * REENCRYPT_CHUNK_SIZE;
              const end = Math.min(start + REENCRYPT_CHUNK_SIZE, totalDecrypted);
              const encrypted = await encryptFile(plaintext.slice(start, end).buffer, newKey);
              await uploadChunkWithRetry(share.id, file.id, encrypted, i, totalChunks);
              if (i < totalChunks - 1) await sleep(INTER_CHUNK_DELAY_MS);
            }
