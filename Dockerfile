...
# Go 1.26.5 fixes CVE-2026-42504, CVE-2026-27145, CVE-2026-42507,
...
# + CVE-2026-39822 et CVE-2026-42505 -- corrigés par Go 1.26.5.
...
# GHSA-qc2q-p7wx-3px3 (MODERATE) : google.golang.org/grpc <= 1.83.0
# CVE-2026-84445 (HIGH) : google.golang.org/grpc 1.83.1 (xDS server DoS)
...
# CVE-2026-37236 : grpc-gateway v2.30.0, mitigated below by disabling
#                  X-HTTP-Method-Override in every ServeMux.
...
RUN go get golang.org/x/net@v0.57.0 \
    ...
    && go mod tidy \
    && go mod edit -require=github.com/grpc-ecosystem/grpc-gateway/v2@v2.30.0 \
...
RUN CGO_ENABLED=0 go build -trimpath -ldflags='-s -w' -o /usr/bin/caddy ./cmd/caddy \
    && go version -m /usr/bin/caddy | grep -Eq \
        'dep[[:space:]]+google[.]golang[.]org/grpc[[:space:]]+v1[.]83[.]2' \
    && go version -m /usr/bin/caddy | grep -Eq \
        'dep[[:space:]]+github[.]com/grpc-ecosystem/grpc-gateway/v2[[:space:]]+v2[.]30[.]0' \
    && go clean -cache -modcache

# ---------------------------
# Stage 0b: Build the minimal runtime init
# ---------------------------
...
# ---------------------------
# Stage 7: Shell-less distroless runtime
# ---------------------------
# base-nossl contains glibc, CA certificates and timezone data, but excludes
...
# Default images are copied without overwriting customized mounted images by
# runtime-init after it prepares and drops to the final volume owner.
...
# Exec form is required because this image intentionally contains no shell.
HEALTHCHECK --interval=10s --timeout=3s CMD ["node", "-e", "const p=process.env.BACKEND_PORT||'8080';const u=process.env.CADDY_DISABLED==='true'?`http://127.0.0.1:${p}/api/health`:'http://127.0.0.1:3000/api/health';fetch(u).then(r=>r.ok||process.exit(1)).catch(()=>process.exit(1))"]

# The init starts as root solely to prepare mounted writable directories. It
# clears supplementary groups and drops to PUID:PGID before execing Node.
USER 0
